...
no hay falsos positivos, la precisión es ≥99% y no queda lectura visual sin resolver.
...
En Cuentas > Movimientos puedes abrir la configuración de clasificación asistida y elegir Gemini, OpenCode Zen o NVIDIA. La clave de cada proveedor se guarda por separado en el llavero del iPhone y solo se usa después de confirmar el envío de gastos pendientes. El proveedor recibe comercio, importe y fecha, nunca cuentas, saldos, ingresos, transferencias ni PDFs. Gemini usa su endpoint oficial compatible con Chat Completions y ofrece `gemini-3.1-flash-lite` y `gemini-3.8-flash`. NVIDIA permite elegir `deepseek-ai/deepseek-v4-flash-0731` o `moonshotai/kimi-k3`; Kimi usa razonamiento bajo, respuesta no transmitida por streaming y un límite corto para evitar esperas innecesarias. Ninguna clave se incluye en el código ni en TestFlight.
