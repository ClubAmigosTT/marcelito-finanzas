...
En Cuentas > Movimientos puedes abrir la configuración de clasificación asistida y elegir Gemini, OpenCode Zen o NVIDIA. La clave de cada proveedor se guarda por separado en el llavero del iPhone y solo se usa después de confirmar el envío de gastos pendientes. El proveedor recibe comercio, importe y fecha, nunca cuentas, saldos, ingresos, transferencias ni PDFs. Gemini usa su endpoint oficial compatible con Chat Completions y ofrece `gemini-3.1-flash-lite` y `gemini-3.8-flash`; ninguna clave se incluye en el código ni en TestFlight.
