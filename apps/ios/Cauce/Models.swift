...
struct StatementSummaryRecord: Codable {
    ...
    /// RappiCard prints payments and credits as one independently audited total.
    var paymentsAndCredits: Decimal? = nil
...
/// with the origin at the bottom-left, while text-layer imports may leave
/// this nil. PDFKit consumes the same bottom-left convention when the
/// viewer focuses the source row.
...
func readerRecoveryLabel(_ value: String) -> String {
    ...
    case "rappi-unsafe-candidate-stream-rejected": return "corriente Rappi no conciliada descartada"
...
struct ImportSummary {
    ...
    var extractionProvider: String = "local"
...
struct StatementAuditRow: Identifiable {
    ...
    /// Rows kept out of the financial book because the document-level gate
    /// did not prove date, amount, sign or column quality. This is distinct
    /// from enrichment rows, which are financially valid but need a merchant
    /// or category confirmation.
    let blockedRows: Int
...
struct LedgerDiagnosticReport {
    ...
    let blockedMovementCount: Int
    /// Actual Movement records kept outside the canonical ledger. This is
    /// intentionally separate from candidate rows that were never persisted.
    let quarantinedMovementCount: Int
    /// Candidate rows retained only in a blocked statement's metadata.
    let quarantinedCandidateCount: Int
    let enrichmentMovementCount: Int
    ...
    var text: String {
        ...
        var lines = [
            ...
            "Movimientos fuera del libro canónico: \(quarantinedMovementCount)",
            "Candidatos fuera de KPI (no persistidos): \(quarantinedCandidateCount)",
...
final class FinanceStore {
    ...
    static func selectableTextLayerDebugForTesting(
        ...
        let rows = source.localizedCaseInsensitiveContains("Amex")
            ? parseAmexText(repaired, fileName: fileName)
            : parse(text: repaired, fileName: fileName, sourceHint: source)
    ...
    /// Runs Santander's fixed table geometry reader against Vision-like
    /// observations. This is the production contract for Santander; the
    /// legacy OCR seam above remains available only for backwards-compatible
    /// fixture diagnostics.
    ...
    static func rappiHybridSelectionForTesting(
        ...
        let selected = reconciledRappiSelection(
            candidateSets: [
                (layoutCandidates, statementSummary),
                (selectableCandidates, statementSummary),
                (duplicateRepairedOCRCandidates, statementSummary),
                (ocrCandidates, statementSummary),
                (repairedOCRCandidates, statementSummary)
            ],
            kind: .card
        )
        return ReaderParseSnapshot(
            ...
            // The test seam follows the production invariant: an unverified
            // OCR stream is evidence for diagnostics, never a returned ledger
            // candidate set.
            movements: selected?.candidates ?? [],
    ...
    /// Runs the native PDF text parser used by production imports. No Vision
    ...
    static func hasIndependentOCRProofForTesting(
        ...
        case "bbva":
            ...
            let movementDiagnostics = diagnostics.filter {
                !isBBVAForeignAuxiliaryDiagnostic($0)
            }
    ...
    /// the diagnostic for a USD/TC/AUT auxiliary line. Keep the exclusion
    /// rule semantic so a clearly identified foreign-currency annotation does
    /// not quarantine an otherwise complete BBVA MXN stream merely because
    /// the device rendered the label differently.
    private static func isBBVAForeignAuxiliaryDiagnostic(_ diagnostic: OCRRowDiagnostic) -> Bool {
        let normalized = diagnostic.reason
        ...
        return compact.hasPrefix("bbvaforeignauxiliary")
            || (compact.contains("bbva") && compact.contains("foreign") && compact.contains("auxiliary"))
    }
    ...
    var ledgerQuality: LedgerQuality {
        ...
        let reviewRows = canonical.filter { movement in
            let categoryNeedsReview = isClassifiableExpenseForCategory(movement)
                && Self.pendingCategoryNames.contains(movement.category)
            return categoryNeedsReview || movement.merchantReviewReason != nil
        }
        let reviewCount = reviewRows.count
        let reviewAmount = reviewRows.reduce(Decimal(0)) { $0 + absolute($1.amount) }
        // A rejected Rappi row is intentionally not persisted in `movements`.
        // Count the candidate rows attached to a blocked statement so the
        // dashboard can explain the quarantine instead of showing only a
        // document-level warning. Once a statement is eligible, only explicit
        ...
        let value = LedgerQuality(
            ...
            enrichmentMovementCount: reviewCount,
            blockedMovementCount: blockedCount,
    ...
    var dashboardIsProvisional: Bool { eligibleMovements.contains(where: isProvisionalScreenshotMovement) }
    var provisionalMovementCount: Int { eligibleMovements.filter(isProvisionalScreenshotMovement).count }
    var provisionalSpend: Decimal { netExpenseMovements.filter(isProvisionalScreenshotMovement).reduce(0) { $0 + $1.expenseContribution } }
    ...
    func diagnosticExportText() -> String {
        ...
        var lines = [
            ...
            "Movimientos fuera del libro canónico: \(quality.quarantinedMovementCount)",
            "Candidatos fuera de KPI (no persistidos): \(quality.quarantinedCandidateCount)",
    ...
    func diagnosticReport() -> LedgerDiagnosticReport {
        ...
        for statement in statements {
            ...
            for (index, row) in (statement.rowDiagnostics ?? []).enumerated()
                where row.accepted && row.reason.hasPrefix("bbva.foreign-auxiliary") {
                var detail = row.reason.trimmingCharacters(in: .whitespacesAndNewlines)
                if let page = row.page {
                    detail = "Página \(page) · \(detail)"
                }
                ...
                add(
                    id: "\(statementID)-foreign-auxiliary-\(index)",
                    severity: reconciliationStatus == .valid ? .warning : .error,
                    source: source,
                    period: period,
                    title: "Evidencia extranjera pendiente",
                    detail: detail
                )
            }
        ...
        // The report is an action list, not a row dump. Multiple identical
        // category or OCR findings for the same merchant/date are collapsed
        // so the user sees the distinct problems without losing any canonical
        // movement or its underlying evidence.
        ...
        issues = issues.filter { issue in
            ...
            return seenIssueKeys.insert(key).inserted
        }
        ...
        return LedgerDiagnosticReport(
            ...
            quarantinedMovementCount: quality.quarantinedMovementCount,
    ...
    var travelSpend: Decimal {
        netExpenseMovements.filter(isTravel).reduce(0) { $0 + $1.expenseContribution }
    }
    var travelPercent: Decimal? { consolidatedRealSpend > 0 && travelSpend >= 0 && travelSpend <= consolidatedRealSpend ? travelSpend / consolidatedRealSpend : nil }
    var ordinarySpend: Decimal { consolidatedRealSpend - travelSpend }
    var ordinaryAverageMonthly: Decimal { cardPeriodCount == 0 ? 0 : ordinarySpend / Decimal(cardPeriodCount) }
    var latestMsiMonthlyLoad: Decimal? { completeSum(latestCompleteMetrics(.card).map(\.msiMonthlyLoad)) }
    var latestMsiOriginalDeferred: Decimal? { completeSum(latestCompleteMetrics(.card).map(\.msiOriginalDeferred)) }
    var latestMsiPending: Decimal? { completeSum(latestCompleteMetrics(.card).map(\.msiPending)) }
    var latestRevolvingBalance: Decimal? { completeSum(latestCompleteMetrics(.card).map(\.revolvingBalance)) }
    var latestMsiInstallmentsCount: Int? { cardPeriodMetrics.first?.msiInstallmentsCount }
    var latestPaymentForNoInterest: Decimal? { completeSum(latestCompleteMetrics(.card).map(\.paymentForNoInterest)) }
    private func latestCompleteMetrics(_ kind: StatementKind) -> [StatementMetric] {
        let values = latestMetricsBySource(periodMetrics.filter { $0.kind == kind })
        let expected = Set(statements.filter { statementKind($0) == kind }.map { $0.accountKey ?? "\($0.source)|\(kind)" })
        let present = Set(values.map { $0.accountKey ?? "\($0.source)|\(kind)" })
        return expected == present ? values : []
    }
    ...
    var consolidatedRealSpend: Decimal {
        netExpenseMovements.reduce(0) { $0 + $1.expenseContribution }
    }
    var totalIncome: Decimal { realIncome }
    var totalTransfers: Decimal { eligibleMovements.filter { $0.flow == .transfer && $0.amount < 0 }.reduce(0) { $0 + absolute($1.amount) } }
    var totalExpenses: Decimal { consolidatedRealSpend }
    var realExpenseMovements: [Movement] { eligibleMovements.filter(isSpend) }
    /// Shared net-spend population for every analytical screen and drilldown.
    var netExpenseMovements: [Movement] {
        eligibleMovements.filter { isSpend($0) || (movementKind($0) == .refund && $0.amount > 0) }
            .map { row in
                var result = row
                result.kind = movementKind(row)
                return result
            }
    }
    ...
    var currentPeriodExpenseMovements: [Movement] {
        netExpenseMovements.filter { movement in
            guard let currentPeriodKey else { return false }
    ...
    var cashAvailable: Decimal? {
        completeSum(latestCompleteMetrics(.bank).map(\.cashBalance))
    }
    var debtTotal: Decimal? {
        completeSum(latestCompleteMetrics(.card).map(\.debtBalance))
    }
    ...
    var liquidPatrimonyChangePercent: Decimal? {
        let values = balanceHistory.compactMap(\.patrimony)
        guard values.count > 1, let current = values.last else { return nil }
    ...
    struct BalanceSnapshot {
        ...
        let cash: Decimal?
        let debt: Decimal?
        var patrimony: Decimal? { cash.flatMap { cash in debt.map { cash - $0 } } }
    }

    /// Common account universe, last official balance carried forward at each
    /// cutoff. Never group bank/card balances by their human-readable labels.
    var balanceHistory: [BalanceSnapshot] {
        let metrics = periodMetrics
        let keys = Set(statements.map { $0.accountKey ?? "\($0.source)|\(statementKind($0))" })
        let dated = metrics.compactMap { metric -> (StatementMetric, Date)? in
            guard let date = periodDate(from: metric.period) else { return nil }
            return (metric, date)
        }
        return Set(dated.map { $0.1 }).sorted().map { date in
            let latest = latestMetricsBySource(dated.filter { $0.1 <= date }.map { $0.0 })
            guard Set(latest.map { $0.accountKey ?? "\($0.source)|\($0.kind)" }) == keys else {
                return BalanceSnapshot(date: date, cash: nil, debt: nil)
            }
            return BalanceSnapshot(date: date,
                cash: completeSum(latest.filter { $0.kind == .bank }.map(\.cashBalance)),
                debt: completeSum(latest.filter { $0.kind == .card }.map(\.debtBalance)))
        }
    }

    var balanceEvidenceDescription: String {
        let metrics = latestMetricsBySource(periodMetrics)
        return metrics.sorted { $0.source < $1.source }.map {
            let metric = $0
            let record = statements.first { $0.id == metric.id }
            var text = "\(metric.source) · \(metric.accountKey ?? "cuenta sin identificar") · \(metric.period)"
            if let summary = record?.summary {
                if metric.kind == .bank, let opening = summary.previousBalance, let deposits = summary.depositTotal,
                   let withdrawals = summary.withdrawalTotal, let closing = summary.cashBalance {
                    text += "\n\(opening) + \(deposits) − \(withdrawals) = \(closing) MXN (apertura + abonos − cargos)"
                } else if metric.kind == .card, let limit = metric.creditLimit, let available = metric.creditAvailable {
                    text += "\n\(limit) − \(available) = \(limit - available) MXN (límite − disponible)"
                }
            }
            return text
        }.joined(separator: "\n")
    ...
    var creditLimit: Decimal? {
        completeSum(latestCompleteMetrics(.card).map(\.creditLimit))
    }
    var creditAvailable: Decimal? {
        completeSum(latestCompleteMetrics(.card).map(\.creditAvailable))
    }
    var creditUsed: Decimal? {
        completeSum(latestCompleteMetrics(.card).map(\.creditUsed))
    }
    ...
    var monthlyExpense: Decimal {
        currentPeriodExpenseMovements.reduce(0) { $0 + $1.expenseContribution }
    }
    ...
    var cashFlowHistory: [CashFlowPoint] {
        ...
        for movement in eligibleMovements {
            ...
            } else if movementKind(movement) == .refund && movement.amount > 0 {
                expenseByDay[day, default: 0] -= movement.amount
            }
        }
    ...
    private func movementKind(_ movement: Movement) -> MovementKind {
        ...
        if movement.account == "Rappi" {
            if value.contains("pago por spei") { return .cardPayment }
            if movement.amount > 0 && value.contains("cashback") { return .refund }
        }
    ...
    private func isRealIncome(_ movement: Movement) -> Bool {
        ...
        if isProvisionalScreenshotMovement(movement),
           screenshotCaptures.first(where: { $0.movements.contains(where: { $0.id == movement.id }) })?.source.kind == .card {
            return false
        }
    ...
    private var currentPeriodKey: String? {
        let components = Calendar(identifier: .gregorian).dateComponents([.year, .month], from: Date.now)
        guard let year = components.year, let month = components.month else { return nil }
        return String(format: "%04d-%02d", year, month)
    }

    private func movementPeriodKey(_ movement: Movement) -> String? {
        // One calendar month for PDF, screenshot and manual rows alike.
    ...
    func normalizeFinanceForTesting() { normalizeStoredLedger() }
    func statementMetricForTesting(_ statement: StatementRecord) -> StatementMetric { calculateMetric(for: statement) }
    ...
    /// Only days explicitly covered by every selected account may contribute
    /// zero to an average. Missing statements never fabricate zero-spend days.
    func spendingCoveredDays(account: String? = nil) -> Set<Date> {
        let calendar = Calendar.current
        let selected = statements.filter { account == nil || $0.source == account }
        let groups = Dictionary(grouping: selected) { $0.accountKey ?? $0.source }
        var common: Set<Date>?
        for records in groups.values {
            var days = Set<Date>()
            for record in records where isEligibleStatement(record) {
                // A month label is not proof of the exact covered days.
                let explicitDate = #"\d{1,2}[/-]\d{1,2}[/-]\d{4}|\d{4}-\d{1,2}-\d{1,2}|\d{1,2}\s+(?:de\s+)?[a-zA-Záéíóú]+\s+(?:de\s+)?\d{4}"#
                let matches = (try? NSRegularExpression(pattern: explicitDate))?.numberOfMatches(in: record.period, range: NSRange(record.period.startIndex..., in: record.period)) ?? 0
                guard matches >= 2, let range = statementRange(from: record.period) else { continue }
                var cursor = calendar.startOfDay(for: range.lowerBound)
                let end = min(calendar.startOfDay(for: range.upperBound), calendar.startOfDay(for: .now))
                while cursor <= end {
                    days.insert(cursor)
    ...
    private func financialAccountKey(_ movement: Movement) -> String? {
        if let statement = statementForMovement(movement), let key = statement.accountKey, !key.isEmpty {
            return "\(statement.source.lowercased())|\(key.lowercased())"
        }
        if let capture = screenshotCaptures.first(where: { $0.movements.contains(where: { $0.id == movement.id }) }),
           let key = capture.accountKey, !key.isEmpty {
            return "\(capture.source.rawValue.lowercased())|\(key.lowercased())"
        }
        return nil
    }

    private func distinctImportedBankAccounts(_ outflow: Movement, _ inflow: Movement) -> Bool {
        guard isBankMovement(outflow), isBankMovement(inflow),
              let outKey = financialAccountKey(outflow), let inKey = financialAccountKey(inflow) else { return false }
        return outKey != inKey
    }
    ...
    private func reconcileStoredMovements() {
        for index in movements.indices where !movements[index].manuallyReviewed && movements[index].reconciliationReason != nil {
            movements[index].matchedMovementId = nil
            movements[index].reconciliationConfidence = nil
            movements[index].reconciliationReason = nil
            movements[index].flow = movements[index].amount > 0 ? .income : .expense
            movements[index].kind = nil
            movements[index].category = Self.category(for: movements[index].title, flow: movements[index].flow)
        }
        ...
        for index in movements.indices {
            ...
            guard !bank.manuallyReviewed, !consumed.contains(bank.id), isBankMovement(bank), isOutflow(bank) else { continue }

            let cardCandidates = movements.indices.filter { candidateIndex in
                let card = movements[candidateIndex]
                guard !card.manuallyReviewed, !consumed.contains(card.id), candidateIndex != index, isCardMovement(card), amountsMatch(bank.amount, card.amount), abs(bank.date.timeIntervalSince(card.date)) <= cardPaymentWindow else { return false }
            ...
            if cardCandidates.count == 1, let cardIndex = cardCandidates.first {
                ...
                movements[cardIndex].flow = .transfer
        ...
        for outflowIndex in movements.indices {
            ...
            guard !outflow.manuallyReviewed, !consumed.contains(outflow.id), isBankMovement(outflow), isOutflow(outflow) else { continue }
            for inflowIndex in movements.indices {
                let inflow = movements[inflowIndex]
                guard !inflow.manuallyReviewed, inflowIndex != outflowIndex,
                ...
                let sharedReference = !transferReferenceTokens(outflow.title).intersection(transferReferenceTokens(inflow.title)).isEmpty
                let ownEvidence = ownerAliasMentioned(in: "\(outflow.title) \(inflow.title)") || sharedReference
                if ownEvidence && evidence.score >= 90 {
        ...
        let unambiguousCandidates = scoredCandidates.filter { candidate in
            !scoredCandidates.contains { other in
                (other.outflowIndex != candidate.outflowIndex || other.inflowIndex != candidate.inflowIndex)
                    && (other.outflowIndex == candidate.outflowIndex || other.inflowIndex == candidate.inflowIndex)
                    && other.score >= candidate.score - 8
            }
        }
        let selectedTransfers = globallyBestTransferCandidates(unambiguousCandidates)
            .sorted { left, right in
        ...
        for index in movements.indices {
            ...
            guard !movement.manuallyReviewed, !consumed.contains(movement.id),
                  isBankMovement(movement),
                  hasTransferHint(movement),
                  ownerAliasMentioned(in: movement.title) else { continue }
            movements[index].reconciliationConfidence = 60
            movements[index].reconciliationReason = "Posible transferencia propia: falta contraparte. Confirma el tipo manualmente; permanece en métricas."
            ambiguousRows.insert(movement.id)
        }

        if !ambiguousRows.isEmpty {
            for index in movements.indices where ambiguousRows.contains(movements[index].id) && movements[index].reconciliationReason == nil {
                movements[index].reconciliationConfidence = 0
                movements[index].reconciliationReason = "Coincidencia de importe/fecha sin identidad suficiente. No se excluyó; revisa si es transferencia propia."
            }
            DiagnosticsRecorder.record(
                level: "info",
    ...
    private func isBankMovement(_ movement: Movement) -> Bool {
        if let statement = statementForMovement(movement) { return statementKind(statement) == .bank }
        return screenshotCaptures.first(where: { $0.movements.contains(where: { $0.id == movement.id }) })?.source.kind == .bank
    }
    ...
    private func calculateMetric(for statement: StatementRecord) -> StatementMetric {
        ...
        let msiFallback = msi.reduce(Decimal(0)) { $0 + absolute($1.amount) }
        ...
        let creditLimit = statement.summary?.creditLimit.flatMap { $0 >= 0 ? $0 : nil }
        let creditAvailable = statement.summary?.creditAvailable.flatMap { $0 >= 0 ? $0 : nil }
        let creditUsed: Decimal?
        ...
        if kind == .card, let creditLimit, let creditAvailable {
            debt = creditLimit - creditAvailable
        } else {
            debt = statement.summary?.debtBalance
                ?? (kind == .card ? statement.summary?.statementBalance : nil)
        }
        ...
        let revolvingBalance = statement.summary?.revolvingBalance.map(absolute)
            ?? debt.flatMap { balance in msiPending.map { max(Decimal(0), balance - $0) } }
        let cash = statement.summary?.cashBalance
        ...
        return StatementMetric(
            ...
            msiMonthlyLoad: statement.summary?.msiMonthlyLoad.map(absolute) ?? (msi.isEmpty ? nil : msiFallback),
            cashBalance: cash,
            debtBalance: debt
        )
    }
    ...
    func updateClassification(for movement: Movement, kind: MovementKind, travelRelated: Bool) {
        ...
        movements[index].classificationTags.removeAll { $0 == "viaje" }
        if travelRelated { movements[index].classificationTags.append("viaje") }
        switch kind {
    ...
    private func manualReviewKey(_ movement: Movement) -> String {
        let identity = financialAccountKey(movement) ?? movement.statementId?.uuidString ?? movement.id.uuidString
        return "\(identity)|\(normalizedDate(movement.date))|\(movement.amount)|\(movement.extractionEvidence?.page ?? 0)|\(movement.extractionEvidence?.sourceText ?? movement.title)"
    }

    private func restoreManualReviews() {
        guard !isReconciliationOnly,
              let data = UserDefaults.standard.data(forKey: "marcelito.manualReviews.v1"),
              let saved = try? JSONDecoder().decode([String: Movement].self, from: data) else { return }
        let counts = Dictionary(grouping: movements, by: manualReviewKey).mapValues(\.count)
        for index in movements.indices {
            guard counts[manualReviewKey(movements[index])] == 1 else { continue }
            guard let prior = saved[manualReviewKey(movements[index])] else { continue }
            movements[index].category = prior.category
            movements[index].kind = prior.kind
            movements[index].flow = prior.flow
            movements[index].travelRelated = prior.travelRelated
            movements[index].classificationTags = prior.classificationTags
            movements[index].manuallyReviewed = true
        }
    }
    ...
    func clearLocalData() {
        UserDefaults.standard.removeObject(forKey: "marcelito.manualReviews.v1")
        movements = []
        statements = []
        screenshotCaptures = []
        ledgerVersion = UUID()
    ...
    func rebuildCanonicalLedgerIfNeededAsync(
        ...
        // Explicit refresh, never cold launch: reconsider old automatic links
        // while preserving source files and individual manual corrections.
        let defaults = UserDefaults.standard
        if defaults.integer(forKey: "marcelito.financialLogicVersion") < 2 {
            restoreManualReviews()
            reconcileStoredMovements()
            reconcileBankScreenshotsAgainstOfficialLedger()
            persist(markingChange: true)
            defaults.set(2, forKey: "marcelito.financialLogicVersion")
        }
    ...
    private func reconcileStatement(
        ...
        if kind == .bank {
            ...
            if let expected = summary.paymentsAndCredits {
                compare("pagos y abonos", extracted: payments + credits, expected: expected)
                if let opening = summary.previousBalance, let closing = summary.statementBalance {
                    let reconstructed = opening + charges - payments - credits
                    if absolute(reconstructed - closing) > tolerance {
                        mismatches.append("saldo final: reconstruido \(reconstructed) vs declarado \(closing)")
                    }
                }
            }
    ...
    private static func extractPDF(
        ...
        if usedOCR, source == "Santander" {
            ...
            let isolatedOCRStream = Self.rappiIsolatedOCRCandidates(
                from: ocrObservations,
                fileName: fileName,
                confidenceByPage: confidenceByPage
            )
            let isolatedOCRCandidates = isolatedOCRStream?.candidates ?? []
            ...
            func recordRappiStream(
                ...
                if name == "ocrWithMergedSummary"
                    ...
                    || name == "isolatedOCRWithMergedSummary"
                    || name == "isolatedSpatialOCRWithMergedSummary"
                    || name == "retryIsolatedOCRWithMergedSummary"
                    || name == "retryIsolatedSpatialOCRWithMergedSummary" {
            ...
            recordRappiStream("isolatedOCRWithMergedSummary", candidates: isolatedOCRCandidates, summary: summary, observations: isolatedOCRStream?.observations)
            recordRappiStream("isolatedSpatialOCRWithMergedSummary", candidates: isolatedSpatialOCRCandidates, summary: summary, observations: isolatedOCRStream?.observations)
            ...
            var selectedRappiCandidates: [Movement] = []
            ...
            if let selected {
                ...
                if selected.candidates.contains(where: { $0.extractionEvidence?.method == "vision-ocr" }) {
                    if let isolatedOCRStream,
                       candidateIDsMatch(selected.candidates, isolatedSpatialOCRCandidates)
                        ...
                        selectedRappiOCRObservations = isolatedOCRStream.observations
                    } else {
                        selectedRappiOCRObservations = ocrObservations
                    }
                }
            } else if allowOCR {
                ...
                let retryIsolatedOCRStream = Self.rappiIsolatedOCRCandidates(
                    from: retryObservations,
                    fileName: fileName,
                    confidenceByPage: retryConfidenceByPage
                )
                let retryIsolatedOCRCandidates = retryIsolatedOCRStream?.candidates ?? []
                ...
                recordRappiStream("retryIsolatedOCRWithMergedSummary", candidates: retryIsolatedOCRCandidates, summary: summary, observations: retryIsolatedOCRStream?.observations)
                recordRappiStream("retryIsolatedSpatialOCRWithMergedSummary", candidates: retryIsolatedSpatialOCRCandidates, summary: summary, observations: retryIsolatedOCRStream?.observations)
                recordRappiSummary("retrySummary", retrySummary)
#endif
                let retrySelection = Self.reconciledRappiSelection(
                    candidateSets: [
                        (retryIsolatedSpatialOCRCandidates, summary),
                        (retryIsolatedOCRCandidates, summary),
                        (pageWiseCandidates, pageWiseSummary),
                        (layoutCandidates, layoutSummary),
                        (selectableCandidates, selectableSummary),
                    ...
                    if retrySelection.candidates.contains(where: { $0.extractionEvidence?.method == "vision-ocr" }) {
                        if let retryIsolatedOCRStream,
                           candidateIDsMatch(retrySelection.candidates, retryIsolatedSpatialOCRCandidates)
                            ...
                            selectedRappiOCRObservations = retryIsolatedOCRStream.observations
                        } else {
                            selectedRappiOCRObservations = retryObservations
                        }
                    }
                } else if retryCandidates.count > ocrCandidates.count {
                    // The retry is useful as forensic evidence even when it
                    // still cannot be certified. It must not become rows.
            ...
            if let selectedRappiOCRObservations {
                rowDiagnostics = Self.rappiOCRRowDiagnostics(
                    observations: selectedRappiOCRObservations,
                    movements: parsedCandidates,
                    fallbackReason: "RappiCard: importe firmado recuperado visualmente y conciliado con controles independientes"
                )
            } else if parsedCandidates.isEmpty && !diagnosticRappiOCRObservations.isEmpty {
                rowDiagnostics = Self.rappiOCRRowDiagnostics(
                    observations: diagnosticRappiOCRObservations,
                    movements: [],
                    fallbackReason: "RappiCard: ninguna corriente completa de lectura concilió; los candidatos OCR quedaron fuera del libro"
                )
            } else {
        ...
        let selectedOCRRows = candidates.filter { $0.extractionEvidence?.method == "vision-ocr" }
        let selectedNonOCRRows = candidates.filter { $0.extractionEvidence?.method != "vision-ocr" }
        ...
        let ocrFallbackNeedsReview = usedOCR && (
            ocrRejectedRowsNeedReview
                || candidates.isEmpty
                // A complete set must come from one source. This should be
                // unreachable after the paired Rappi selection, but keep the
                // invariant at the import boundary as well.
                || (!selectedOCRRows.isEmpty && !selectedNonOCRRows.isEmpty)
                || candidates.contains {
    ...
    private func persist(markingChange: Bool = false) {
        ...
        let defaults = UserDefaults.standard
        if markingChange {
            var saved = defaults.data(forKey: "marcelito.manualReviews.v1").flatMap { try? JSONDecoder().decode([String: Movement].self, from: $0) } ?? [:]
            for movement in movements where movement.manuallyReviewed { saved[manualReviewKey(movement)] = movement }
            if let data = try? JSONEncoder().encode(saved) { defaults.set(data, forKey: "marcelito.manualReviews.v1") }
        }
    ...
    */]
    ...
    /// One reconstructed Rappi row. Keeping its visual band alongside the
    /// serialized text lets the parser preserve a PDF location even when the
    /// OCR stream is later re-grouped into ordinary text lines.
    private struct RappiOCRMovementLine {
        let text: String
        let y: CGFloat
        let bounds: CGRect
        let amount: String
    ...
    private static func rappiOCRRowDiagnostics(
        observations: [OCRObservation],
        movements: [Movement],
        fallbackReason: String
    ) -> [OCRRowDiagnostic] {
        let accepted = rowDiagnostics(for: movements, fallbackReason: fallbackReason)
        let movementPages = Dictionary(grouping: observations.filter { $0.page > 0 }, by: \.page)
            .keys
            .sorted()
        let visualLines = movementPages.flatMap { page in
            rappiOCRMovementLineRecords(from: observations.filter { $0.page == page })
                .map { (page: page + 1, line: $0) }
        }

        // The plain-text fallback on an OCR page is intentionally not enough
        // for an automatic import: without a reconstructed visual row there
        // is no proof that date, merchant and amount came from the same line.
        if visualLines.isEmpty {
            let source = observations
                .filter { $0.page > 0 }
                .sorted { $0.page == $1.page ? $0.centerY > $1.centerY : $0.page < $1.page }
                .prefix(8)
                .map(\.text)
                .joined(separator: " ")
            return accepted + [OCRRowDiagnostic(
                page: observations.first(where: { $0.page > 0 }).map { $0.page + 1 },
                rawText: source.isEmpty ? "Rappi: no se reconstruyó ninguna fila visual" : source,
                selectedColumn: "MONTO MXN",
                reason: "rappi.visual-row-not-reconstructed: la lectura OCR no demostró una fila con fecha e importe en columnas reconocibles",
                accepted: false
            )]
        }

        func decimalEquals(_ left: Decimal?, _ right: Decimal?) -> Bool {
            guard let left, let right else { return false }
            return left == right
        }

        func matches(_ line: (page: Int, line: RappiOCRMovementLine), _ movement: Movement) -> Bool {
            guard let evidence = movement.extractionEvidence,
                  evidence.method == "vision-ocr",
    ...
    /// PDF or a Vision run. `allRows` represents the visual row inventory and
    /// `selectedRows` represents the single OCR stream that reconciled. A row
    /// present only in the former must remain an explicit rejected diagnostic.
    static func rappiOCRRowDiagnosticsForTesting(
        allRows: [String],
        selectedRows: [String],
        page: Int = 2,
        spacing: CGFloat = 0.04
    ) -> [OCRRowDiagnostic] {
        func observations(for texts: [String]) -> [OCRObservation] {
        ...
        let allObservations = observations(for: allRows)
        var remainingSelectedRows = selectedRows
        let selectedObservations = allObservations.filter { observation in
            guard let index = remainingSelectedRows.firstIndex(of: observation.text) else { return false }
            remainingSelectedRows.remove(at: index)
            return true
        }
        let selectedText = rappiOCRText(from: selectedObservations)
        let selectedMovements = parseRappiText(
            selectedText,
            evidenceMethod: "vision-ocr",
            confidenceByPage: [page + 1: 0.99]
        )
        return rappiOCRRowDiagnostics(
            observations: allObservations,
            movements: selectedMovements,
            fallbackReason: "prueba de cobertura Rappi"
        )
    }
            ...
            autoreleasepool {
                ...
                if prioritizeNumericEvidence, pageIndex > 0 {
                    ...
                    if !visualRows.isEmpty {
                        ...
                        // coverage. Keep the full-page reading as well so a
                        // missed band can be recovered by the numeric pass.
                        // `rappiOCRMovementLineRecords` merges only the same
                        // amount at the same vertical position; legitimate
                        // equal purchases at different positions survive.
                        selectedObservations.append(contentsOf: visualRows)
                    }
                }
    ...
    /// Builds a candidate stream exclusively from the bounded row crops. A
    /// full-page Vision pass can emit several readings for one printed row;
    /// the isolated observations keep one physical band per candidate and
    /// therefore provide a useful independent recovery path on devices whose
    /// page-level grouping differs from the simulator. The accounting gate
    /// still decides whether this stream is usable.
    private static func rappiIsolatedOCRCandidates(
        from observations: [OCRObservation],
        fileName: String,
        confidenceByPage: [Int: Double]
    ) -> (candidates: [Movement], observations: [OCRObservation])? {
        let isolated = observations
            .filter { $0.page > 0 && $0.isolatedRappiRow }
            .sorted {
                $0.page == $1.page ? $0.centerY > $1.centerY : $0.page < $1.page
            }
        guard !isolated.isEmpty else { return nil }
        let text = rappiOCRText(from: isolated)
        let candidates = parseRappiText(
            text,
            fileName: fileName,
            evidenceMethod: "vision-ocr",
            confidenceByPage: confidenceByPage
    ...
    /// Merge only overlapping isolated bands that carry the same date and
    /// signed amount. Different vertical bands remain separate, so two real
    /// purchases with the same amount cannot be collapsed. This repairs a
    /// duplicated rule/date region without choosing an arbitrary subset of
    /// unrelated transactions.
    private static func rappiIsolatedSpatiallyDeduplicatedCandidates(
        _ candidates: [Movement]
    ) -> [Movement] {
        func isBetter(_ candidate: Movement, than existing: Movement) -> Bool {
            let candidateHasMerchant = candidate.title.rangeOfCharacter(from: .letters) != nil
            let existingHasMerchant = existing.title.rangeOfCharacter(from: .letters) != nil
            if candidateHasMerchant != existingHasMerchant { return candidateHasMerchant }
            return (candidate.extractionEvidence?.confidence ?? 0) > (existing.extractionEvidence?.confidence ?? 0)
        }

        func overlapRatio(_ left: CGRect, _ right: CGRect) -> CGFloat {
        ...
        for candidate in candidates {
            guard candidate.extractionEvidence?.method == "vision-ocr",
                  let evidence = candidate.extractionEvidence,
                  let page = evidence.page,
                  let selectedAmount = evidence.selectedAmount,
                  let bounds = evidence.bounds else {
            ...
            let candidateRect = CGRect(x: bounds.x, y: bounds.y, width: bounds.width, height: bounds.height)
            if let index = result.firstIndex(where: { existing in
                guard let existingEvidence = existing.extractionEvidence,
                      existingEvidence.method == "vision-ocr",
                      existingEvidence.page == page,
                      existingEvidence.selectedAmount == selectedAmount,
                      existing.date == candidate.date,
                      existing.flow == candidate.flow,
                      let existingBounds = existingEvidence.bounds else { return false }
                ...
                return abs(existingRect.midY - candidateRect.midY) <= 0.014
                    && overlapRatio(existingRect, candidateRect) >= 0.65
            }) {
                if isBetter(candidate, than: result[index]) { result[index] = candidate }
            } else {
                result.append(candidate)
            }
        }
    ...
    private static func rappiOCRText(from observations: [OCRObservation]) -> String {
        ...
        return observationsByPage.keys.sorted().flatMap { page in
            ...
            if page == 0 {
                ...
                let movementLines = rappiOCRMovementLineRecords(from: pageObservations)
    ...
    private static func rappiOCRMovementLines(from observations: [OCRObservation]) -> [String] {
        rappiOCRMovementLineRecords(from: observations).map(\.text)
    }

    private static func rappiOCRMovementLineRecords(from observations: [OCRObservation]) -> [RappiOCRMovementLine] {
        ...
        if observations.count > 1, observations.allSatisfy(\.isolatedRappiRow) {
            return observations.sorted { $0.centerY > $1.centerY }
                .flatMap { rappiOCRMovementLineRecords(from: [$0]) }
        }
        ...
        func titleForRow(y: CGFloat, amount: OCRTextBox) -> String {
            ...
            // If Vision cannot produce a merchant-shaped title, keep the
            // bounded source fragment as the display value. A synthetic label
            // would hide the only evidence the user has for reviewing the row.
        ...
        let rawLines = rows.sorted { $0.y > $1.y }.compactMap { row -> RappiOCRMovementLine? in
            ...
            let rowBounds = observations
                ...
                .map(\.boundingBox)
                .reduce(row.amount.boundingBox) { $0.union($1) }
        ...
        for candidate in rawLines {
            if let index = unique.firstIndex(where: {
                abs($0.y - candidate.y) <= 0.018
                    && normalized($0.amount) == normalized(candidate.amount)
            }) {
                if candidate.isolated && !unique[index].isolated {
    ...
    private static func parseBBVASelectableText(
        ...
        func isForeignAuxiliaryAmount(_ match: TextMatch, in value: String) -> Bool {
            guard let range = Range(match.range, in: value) else { return false }
            let before = String(value[..<range.lowerBound].suffix(24))
                .folding(options: [.diacriticInsensitive, .caseInsensitive], locale: .current)
                .replacingOccurrences(of: #"[^a-z0-9]+"#, with: "", options: .regularExpression)
            let after = String(value[range.upperBound...].prefix(24))
                .folding(options: [.diacriticInsensitive, .caseInsensitive], locale: .current)
                .replacingOccurrences(of: #"[^a-z0-9]+"#, with: "", options: .regularExpression)
            // The foreign token is tied to the amount immediately before
            // `TC/AUT`, or to an amount immediately preceded by `USD`. Do not
            // mark a domestic amount as auxiliary merely because the same
            // row later contains a separate `USD 2.22TC...` continuation.
            return before.hasSuffix("usd")
                || after.hasPrefix("tc")
                || after.hasPrefix("aut")
        }
        ...
        return rows.compactMap { rawRow -> Movement? in
            ...
            let allMoneyMatches = allMatches(in: working, regex: amountRegex)
            let moneyMatches = foreignAuthorization
                ? allMoneyMatches.filter { !isForeignAuxiliaryAmount($0, in: working) }
                : allMoneyMatches
            // A line such as `USD 21.00 TC... AUT...` is auxiliary foreign
            // evidence, not an MXN bank movement. If the row also contains a
            // domestic amount, choose that amount; otherwise keep the row out
            // of the financial candidate stream for manual review.
            guard let firstAmount = moneyMatches.first,
    ...
    /// Deterministic native-text reader for American Express. Amex's PDF
    /// text layer carries the domestic, foreign-currency, payment/credit and
    /// MSI sections as real text; scope and row reconstruction are therefore
    ...
    private static func parseBBVAOCRResult(
        ...
        func isForeignAuxiliaryRow(_ row: [OCRObservation]) -> Bool {
            let raw = row.map(\.text).joined(separator: " ")
            let normalized = raw
                .folding(options: [.diacriticInsensitive, .caseInsensitive], locale: .current)
            let compact = normalized.replacingOccurrences(of: #"[^a-z0-9]+"#, with: "", options: .regularExpression)
            let foreignCurrency = normalized.contains("usd")
                || compact.contains("dls")
                || normalized.contains("moneda extranjera")
            let exchangeRate = compact.contains("tc")
                || compact.contains("tipodecambio")
                || compact.contains("tipocambio")
            let authorization = compact.contains("aut")
                || compact.contains("autoriz")
            return foreignCurrency && exchangeRate && authorization
        }
                ...
                if let result = parseBBVAOCRRow(
                    ...
                    diagnostics.append(
                        OCRRowDiagnostic(
                            ...
                            accepted: true,
                            rowBounds: extractionBounds(for: row)
                        )
                    )
                    previousRunningBalance = result.runningBalance
                } else {
                    let foreignAuxiliary = isForeignAuxiliaryRow(row)
                    diagnostics.append(
                        OCRRowDiagnostic(
                            page: row.first.map { $0.page + 1 },
                            rawText: rawText,
                            selectedColumn: foreignAuxiliary ? "MONEDA EXTRANJERA (USD/TC/AUT)" : nil,
                            reason: foreignAuxiliary
                                ? "bbva.foreign-auxiliary: se conservó evidencia USD/TC/AUT, pero no se inventó un importe MXN"
                                : "fila BBVA rechazada: fecha, importe, dirección de columna o descripción no demostrables",
                            accepted: foreignAuxiliary,
                            rowBounds: extractionBounds(for: row),
                            cellTexts: foreignAuxiliary ? ["USD/TC/AUT sin importe MXN demostrable"] : nil
                        )
                    )
    ...
    private static func parseBBVAOCRRow(
        ...
        let compactFullText = normalizedFullText.replacingOccurrences(of: #"\s+"#, with: "", options: .regularExpression)
        let foreignAuthorization = normalizedFullText.contains("usd")
            && compactFullText.contains("tc")
            && compactFullText.contains("aut")
        let ignoredPhrases = [
            "estado de cuenta", "resumen de cuenta", "resumen de movimientos",
        ...
        let movementAmountCandidates: [OCRAmountCandidate] = {
            guard foreignAuthorization else { return amountCandidates }
            let filtered = amountCandidates.filter { candidate in
                let observation = row[candidate.observationIndex]
                let text = observation.text
                let start = String.Index(
                    utf16Offset: min(candidate.characterOffset, text.utf16.count),
                    in: text
                )
                let prefix = String(text[..<start].suffix(36))
                let suffix = String(text[start...].prefix(36))
                let compactPrefix = prefix
                    .folding(options: [.diacriticInsensitive, .caseInsensitive], locale: .current)
                    .replacingOccurrences(of: #"[^a-z0-9]+"#, with: "", options: .regularExpression)
                let compactSuffix = suffix
                    .folding(options: [.diacriticInsensitive, .caseInsensitive], locale: .current)
                    .replacingOccurrences(of: #"[^a-z0-9]+"#, with: "", options: .regularExpression)
                let isAuxiliary = compactPrefix.hasSuffix("usd")
                    || compactSuffix.hasPrefix("tc")
                    || compactSuffix.hasPrefix("aut")
                return !isAuxiliary
            }
            return filtered
        }()
        // A USD/TC/AUT line without a domestic amount is an auxiliary
        // foreign-currency authorization. It is retained in diagnostics but
        // cannot become a MXN movement by treating USD as pesos.
        guard !movementAmountCandidates.isEmpty else { return nil }

        let orderedAmountCandidates = movementAmountCandidates.sorted { $0.order < $1.order }
        ...
        let movementCandidates = movementAmountCandidates.filter { $0.x >= 0.36 && $0.x < columns.balanceMinX }
    ...
    private static func parseSantanderTable(
        ...
        recoverMissingDateRows: Bool = false
        ...
        var renderedDateAnchors: [DateAnchor] = []
        func highResolutionDateAnchors(in image: CGImage, pageIndex: Int) -> [DateAnchor] {
            ...
            renderedDateAnchors = (request.results ?? []).compactMap { result -> DateAnchor? in
                guard let candidate = result.topCandidates(1).first else { return nil }
                let text = candidate.string
                let range = NSRange(text.startIndex..<text.endIndex, in: text)
                let boundedDate = dateRegex.firstMatch(in: text, range: range).flatMap { match -> (text: String, box: CGRect)? in
                    ...
                    return (String(text[textRange]), box.boundingBox)
                }
                ...
                let isDateCell = hasDate
                    && (boundedDate?.box.midX ?? result.boundingBox.midX) <= dateMaxX
                    && !normalized.contains("periodo")
                    && !normalized.contains("corte")
                    && !normalized.contains("pagina")
                guard isDateCell,
                      let dateText = boundedDate?.text ?? dateRegex.firstMatch(in: text, range: range).flatMap({ match in
                          guard let textRange = Range(match.range, in: text) else { return nil }
                          return String(text[textRange])
                      }) else { return nil }
                let dateBox = boundedDate?.box ?? result.boundingBox
                let source = OCRObservation(
                    page: pageIndex,
                    text: dateText,
                    boundingBox: dateBox,
                    confidence: Double(candidate.confidence),
                    dateBoxes: [OCRTextBox(text: dateText, boundingBox: dateBox, confidence: Double(candidate.confidence))]
                )
                return DateAnchor(
                    box: dateBox,
                    text: dateText,
        ...
        func highResolutionBand(near currentBand: CGRect, in image: CGImage, pageIndex: Int) -> CGRect? {
            ...
            guard let index = anchors.indices.min(by: {
                abs(anchors[$0].box.midY - currentBand.midY) < abs(anchors[$1].box.midY - currentBand.midY)
            }), abs(anchors[index].box.midY - currentBand.midY) <= 0.05 else { return nil }
            let anchor = anchors[index].box
            ...
            let lower = index + 1 < anchors.count ? anchors[index + 1].box.maxY + 0.002 : 0
        ...
        func highResolutionImage(for pageIndex: Int) -> CGImage? {
            guard let document, let page = document.page(at: pageIndex) else { return nil }
            if renderedPage != pageIndex {
                let bounds = page.bounds(for: .mediaBox)
                let scale = min(
                    3200 / max(bounds.width, bounds.height),
                    sqrt(5_000_000 / max(bounds.width * bounds.height, 1))
                )
                renderedImage = page.thumbnail(
        ...
        func retryCells(_ physical: inout PhysicalRow) {
            guard !physical.retried, let document, let pageIndex = physical.observations.first?.page,
                  document.page(at: pageIndex) != nil else { return }
            physical.retried = true
            physical.retryOutcome = "render-unavailable"
            _ = highResolutionImage(for: pageIndex)
        ...
        for page in byPage.keys.sorted() {
            ...
            var anchors = pageObservations.compactMap { observation -> DateAnchor? in
                let normalized = observation.text.folding(options: [.diacriticInsensitive, .caseInsensitive], locale: .current)
            ...
            if recoverMissingDateRows,
               let image = highResolutionImage(for: page) {
                let upperTableY = page == titleAnchor.page ? titleAnchor.lowerY + 0.020 : 1
                let lowerTableY = page == endAnchor.page ? endAnchor.centerY - 0.020 : 0
                let recovered = highResolutionDateAnchors(in: image, pageIndex: page).filter { candidate in
                    candidate.box.maxY <= upperTableY
                        && candidate.box.minY >= lowerTableY
                        && candidate.box.midX <= dateMaxX
                        && !anchors.contains { existing in
                            abs(existing.box.midY - candidate.box.midY) <= 0.012
                        }
                }
                if !recovered.isEmpty {
                    anchors.append(contentsOf: recovered)
                    anchors.sort { $0.box.midY > $1.box.midY }
                }
            }
    ...
    private static func parseSantanderRow(
        ...
        let titleParts = row
            .filter {
                let bounds = titleBounds ?? (min: CGFloat(0.18), max: CGFloat(0.62))
                return $0.centerX >= bounds.min && $0.centerX < bounds.max
            }
            .filter { observation in
                let normalized = observation.text
                    .folding(options: [.diacriticInsensitive, .caseInsensitive], locale: .current)
    ...
    private static func rappiMerchantIdentity(_ value: String, rawDescription: String? = nil) -> RappiMerchantIdentity {
        let raw = value
            .replacingOccurrences(of: #"\s+"#, with: " ", options: .regularExpression)
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let rawEvidence = (rawDescription ?? value)
            .replacingOccurrences(of: #"\s+"#, with: " ", options: .regularExpression)
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let folded = raw
            .folding(options: [.diacriticInsensitive, .caseInsensitive], locale: Locale(identifier: "es_MX"))
        ...
        let reviewReason: String? = likelyOpaqueProcessorDescriptor
            ...
            : nil
        return RappiMerchantIdentity(rawDescription: rawEvidence, normalizedMerchant: normalized, displayMerchant: display, confidence: confidence, reviewReason: reviewReason)
    }

    /// High-signal category hints for compact Rappi descriptors.  This stays
    /// issuer-scoped: an ambiguous token from another bank must not acquire a
    /// category merely because it happens to contain the same substring.
    private static func rappiCategory(
        for merchant: RappiMerchantIdentity,
        rawDescription: String,
        flow: FlowKind,
        kind: MovementKind
    ) -> String? {
        guard flow == .expense, kind == .purchase else { return nil }
        let text = categoryText("\(rawDescription) \(merchant.normalizedMerchant)")
        let words = text.split(separator: " ").map(String.init)
        func hasPrefix(_ value: String) -> Bool { text.hasPrefix(value) }
        func hasWord(_ value: String) -> Bool { categoryContains(text, [value]) }

        if text.contains("google cloud") || text.contains("apple com bill") { return "Software y suscripciones" }
        if hasWord("swappedcom") { return "Comisiones y finanzas" }
        if hasWord("pase") || text.contains("ado web") { return "Transporte" }
        if text.contains("avianca") { return "Viajes" }
        if hasPrefix("oxxo") || hasPrefix("7eleven") || hasPrefix("7 eleven") { return "Tiendita" }
        if text.contains("farm san pablo") || text.contains("farm dr ahorro")
            || text.contains("farm guad") || text.contains("f ahorro")
            || text.contains("multifarmacias") || text.contains("dr ismael") {
            return "Salud"
        }
        if hasWord("sumesa") || text.contains("super fasti") || text.contains("marcas nestle") || hasWord("wildfork") {
            return "Despensa / supermercado"
        }
        if hasWord("decathlon") || text.contains("paddeo sport") || text.contains("estadio azul") {
            return "Deporte"
        }
        if text.contains("lucha libre") || hasWord("cinetec") || hasWord("flix") || hasWord("pingpod") || text.contains("aerodiverti") {
            return "Entretenimiento"
        }
        if hasWord("shein") || hasWord("fraiche") || hasWord("juguete") || hasWord("bout") || text.contains("lib rosario") {
    ...
    private static func rappiCapture(_ pattern: String, in text: String) -> String? {
        let value = text.folding(options: [.diacriticInsensitive, .caseInsensitive], locale: Locale(identifier: "es_MX"))
    ...
    private static func parseRappiText(
        ...
        func flush() {
            ...
            guard let rowPrefixRegex,
                  ...
                  let bodyRange = Range(match.range(at: 3), in: pending) else { return }
            let firstDate = parseDate(String(pending[firstRange]))
            let secondDate: Date? = {
                let secondNSRange = match.range(at: 2)
                guard secondNSRange.location != NSNotFound,
                      let secondRange = Range(secondNSRange, in: pending) else { return nil }
                return parseDate(String(pending[secondRange]))
            }()
            // Rappi prints operation date before charge date, but Vision can
            // return those columns reversed on a foreign-currency row. Keep
            // the earlier date only for a plausible posting gap. A much
            // earlier second token is commonly a truncated OCR date such as
            // `2026-05-2`; in that case the first column remains the reliable
            // operation date.
            let date: Date?
            if let firstDate {
                if let secondDate, secondDate < firstDate {
                    let gapDays = abs(Calendar(identifier: .gregorian).dateComponents([.day], from: secondDate, to: firstDate).day ?? Int.max)
                    date = gapDays <= 7 ? secondDate : firstDate
                } else {
                    date = firstDate
                }
            } else {
            ...
            for (index, candidate) in candidateRanges.enumerated() {
                ...
                let rappiCategoryHint = rappiCategory(
                    for: merchant,
                    rawDescription: titleBody,
                    flow: flow,
                    kind: kind
                )
    ...
    /// printed statement contains one row.  Keep one candidate when the
    /// evidence points to the same physical band, but never collapse two
    /// legitimate identical purchases that occupy different bands.
    private static func rappiOCRSpatiallyDeduplicatedCandidates(_ candidates: [Movement]) -> [Movement] {
        ...
        func isBetter(_ candidate: Movement, than existing: Movement) -> Bool {
            let candidateEvidence = candidate.extractionEvidence
            let existingEvidence = existing.extractionEvidence
            let candidateHasMerchant = candidate.title.rangeOfCharacter(from: .letters) != nil
            let existingHasMerchant = existing.title.rangeOfCharacter(from: .letters) != nil
            if candidateHasMerchant != existingHasMerchant { return candidateHasMerchant }
            return (candidateEvidence?.confidence ?? 0) > (existingEvidence?.confidence ?? 0)
        }
        ...
        for candidate in candidates {
            guard candidate.extractionEvidence?.method == "vision-ocr",
                  let candidateEvidence = candidate.extractionEvidence,
                  let candidatePage = candidateEvidence.page,
                  let candidateBounds = candidateEvidence.bounds,
                  let candidateAmount = candidateEvidence.selectedAmount else {
                result.append(candidate)
                continue
            }

            let duplicateIndex = result.firstIndex { existing in
                guard existing.extractionEvidence?.method == "vision-ocr",
                      let existingEvidence = existing.extractionEvidence,
                      existingEvidence.page == candidatePage,
                      existingEvidence.selectedAmount == candidateAmount,
                      existing.date == candidate.date,
                      existing.kind == candidate.kind,
                      existing.flow == candidate.flow,
                      existing.foreignCurrency == candidate.foreignCurrency,
                      let existingBounds = existingEvidence.bounds else { return false }
                let candidateCenterY = candidateBounds.y + candidateBounds.height / 2
                let existingCenterY = existingBounds.y + existingBounds.height / 2
                guard abs(candidateCenterY - existingCenterY) <= 0.022 else { return false }
                let candidateSource = compact(candidateEvidence.sourceText ?? candidate.title)
                let existingSource = compact(existingEvidence.sourceText ?? existing.title)
                let sameMerchant = compact(candidate.title) == compact(existing.title)
                    || (!candidateSource.isEmpty && candidateSource == existingSource)
                // Geometry is necessary but not sufficient: two legitimate
                // adjacent purchases can share date, amount and direction.
                // Require merchant/source identity before collapsing a band;
                // numeric-only duplicates are handled by the occurrence
                // budget below when they share the same OCR source fragment.
                return sameMerchant
            }

            if let duplicateIndex {
                if isBetter(candidate, than: result[duplicateIndex]) {
    ...
    /// The occurrence budget remains useful when one OCR observation contains
    /// a repeated signed token. It runs after the spatial pass so an isolated
    /// row and its full-page counterpart cannot survive as two movements.
    private static func rappiOCRDuplicateRepairedCandidates(_ candidates: [Movement]) -> [Movement] {
        ...
        let spatiallyDeduplicated = rappiOCRSpatiallyDeduplicatedCandidates(candidates)
        var grouped: [GroupKey: [Int]] = [:]
        for (index, candidate) in spatiallyDeduplicated.enumerated() {
            guard candidate.extractionEvidence?.method == "vision-ocr",
        ...
        var keep = Array(repeating: true, count: spatiallyDeduplicated.count)
        for (key, indexes) in grouped where indexes.count > 1 {
            ...
            for index in indexes {
                let title = spatiallyDeduplicated[index].title
                guard title.rangeOfCharacter(from: .letters) == nil else { continue }
                let hasMerchantSibling = indexes.contains { siblingIndex in
                    siblingIndex != index
                        && spatiallyDeduplicated[siblingIndex].date == spatiallyDeduplicated[index].date
                        && spatiallyDeduplicated[siblingIndex].amount == spatiallyDeduplicated[index].amount
                        && spatiallyDeduplicated[siblingIndex].flow == spatiallyDeduplicated[index].flow
                        && spatiallyDeduplicated[siblingIndex].title.rangeOfCharacter(from: .letters) != nil
                }
            ...
            for index in indexes {
                ...
                let title = compact(spatiallyDeduplicated[index].title)
    ...
    private static func reconciledRappiSelection(
        candidateSets: [([Movement], StatementSummaryRecord?)],
        kind: StatementKind
    ) -> (candidates: [Movement], summary: StatementSummaryRecord)? {
        let verifier = FinanceStore(reconciliationOnly: true)
        for (candidates, summary) in candidateSets where !candidates.isEmpty {
    ...
    private static func rappiEvidenceBackedFallbackCandidates(_ candidates: [Movement]) -> [Movement] {
        candidates.map { candidate in
            ...
            // Preserve the source fragment instead of inventing a merchant
            // name. The row may enter the book only when the complete stream
            // reconciles, but it remains explicitly actionable for review.
            repaired.category = "Por revisar"
            if repaired.merchantReviewReason == nil {
                repaired.merchantReviewReason = "No se pudo identificar un comercio con confianza; se conserva el texto original."
            }
            if let source = [
                candidate.rawDescription,
                candidate.extractionEvidence?.sourceText,
                candidate.title
            ].compactMap({ value -> String? in
                guard let value, !value.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty else { return nil }
                return value.trimmingCharacters(in: .whitespacesAndNewlines)
            }).first {
                repaired.displayMerchant = String(source.prefix(120))
            }
            return repaired
        }
    }
    ...
    private static func rappiSummary(_ text: String) -> StatementSummaryRecord? {
        ...
        result.statementBalance = money("saldo deudor total")
        result.debtBalance = result.statementBalance
        result.newCharges = money(#"cargos regulares \(no a meses\)"#)
        ...
        result.msiMonthlyLoad = money(#"cargos compras a meses \(capital\)"#)
        result.interest = money("monto de intereses")
        result.fees = money("monto de comisiones")
        // This reader supports the supplied regular-purchase layout only.
        // A future MSI-bearing layout must not be silently accepted.
        guard result.previousBalance != nil, result.statementBalance != nil,
              result.paymentsAndCredits != nil, result.newCharges != nil,
              result.msiMonthlyLoad == 0, result.msiPending == 0 else { return nil }
        return result
    }

    private static func summary(from text: String, source: String) -> StatementSummaryRecord? {
        if source == "Rappi" { return rappiSummary(text) }
        let normalized = text.folding(options: [.diacriticInsensitive, .caseInsensitive], locale: .current)
        let range = NSRange(normalized.startIndex..<normalized.endIndex, in: normalized)
        func normalizeBareBankAmount(_ raw: String, _ parsed: Decimal) -> Decimal {
            ...
            // OCR can collapse both decimal separators in bank totals
            // (example: 6416111). Keep this recovery bounded so account
        ...
        // total (for example, a fused OCR token). The generic helper
    ...
    private static func category(for title: String, flow: FlowKind) -> String {
        ...
        let rules: [(String, [String])] = [
            ...
            ("Compras personales", ["apple", "shein", "amazon", "sanborns", "miniso", "old navy", "mercadolibre", "mercado libre", "mercadopago", "office depot", "lumen", "steren", "boutique", "tienda", "shop", "store", "ropa", "zapateria", "departamental", "electronic", "electronico", "accesorio", "compras"])
        ]
