...
La referencia exacta del código que corresponde al build 202 es `544d7ae`, con cambios de implementación en `00c6cfe`, `4d57dee` y `2d1cf1b`; el lector conserva la versión declarada `ios-reader-deterministic-2026.09.15.39`. El tag `ios-v1.0.4-bootstrap` apunta a ese head y es el artefacto actual para la prueba física; no se debe tomar `ios-v1.0.3-bootstrap`, `ios-v1.0.110` ni `ios-v1.0.111` como artefacto actual. Antes de compilar, Luna debe comprobar `git diff --stat 6584fc3..HEAD` y revisar cualquier cambio posterior fuera de `docs/`, `package.json`, `.github/workflows/ios-testflight.yml`, `scripts/verify-native-*.ts` y las pruebas de contrato. Una build posterior requiere volver a ejecutar la validación completa.
