...
La referencia exacta del runtime iOS que corresponde al build 201 es `6584fc3`. El tag `ios-v1.0.3-bootstrap` apunta a `2f41c1c`, que además incorpora documentación, gates y workflow de entrega; no se debe tomar `ios-v1.0.110` ni `ios-v1.0.111` como artefacto actual. Antes de compilar, Luna debe comprobar `git diff --stat 6584fc3..HEAD` y revisar cualquier cambio posterior fuera de `docs/`, `package.json`, `.github/workflows/ios-testflight.yml`, `scripts/verify-native-*.ts` y las pruebas de contrato. El build 201 ya contiene el runtime de `6584fc3`; una build posterior requiere volver a ejecutar la validación completa.

En GitHub, el enlace de referencia de esa ejecución es:
`https://github.com/ClubAmigosTT/marcelito-finanzas/actions/runs/35067980061`
