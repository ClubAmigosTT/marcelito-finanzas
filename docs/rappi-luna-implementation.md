...
Importante para esta entrega: la pantalla genérica `Certificar estados con Vision` de la app tiene una compuerta amplia de al menos 10 archivos y no sustituye el manifiesto privado con goldens de Rappi. Para estos seis PDFs, usar el runner anterior con `MARCELITO_PDF_CORPUS_MANIFEST` y conservar el informe/log fuera del repositorio. Solo después de obtener `certified=true` para el conjunto exacto, registrar en GitHub `MARCELITO_NATIVE_CORPUS_CERTIFIED=true` y `MARCELITO_NATIVE_CORPUS_READER_VERSION=ios-reader-deterministic-2026.09.15.39`. No subir un `docs/native-corpus-certification.json` de seis archivos: el workflow de TestFlight lo interpreta como informe genérico y le exige 10.
