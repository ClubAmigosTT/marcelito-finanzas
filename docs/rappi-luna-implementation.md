...
La versión vigente del lector es `ios-reader-recovery-2026.09.25.14`; mantiene el trabajo PDF/Vision serializado, las cancelaciones entre páginas y las compuertas de identidad, conciliación y filas. El contrato compartido sigue en `rappi-shared-engine-2026.09.23.1`. La certificación previa ya no corresponde y no habilita una publicación nueva; hay que repetir el corpus con la versión actual.
...
- Versión del lector que debe aparecer en los próximos informes: `ios-reader-recovery-2026.09.25.14`.
...
- La ruta OCR de Rappi conserva diagnóstico por cada fila visual no seleccionada. Si no existe una fila geométrica con fecha e importe, se registra `rappi.visual-row-not-reconstructed`; si una corriente completa concilió pero dejó fuera una fila visual, se registra `rappi.visual-row-unselected`; si ninguna corriente concilió, se registra `rappi.visual-stream-unreconciled` y cada candidato permanece fuera del libro. El diagnóstico ya no presenta como ilegible una fila que sí conservó fecha, importe y descripción.
...
- El informe nativo distingue la señal OCR agregada de página de la confianza financiera por fila. Para Rappi, las páginas débiles son contexto no bloqueante; el informe nombra la causa real cuando una corriente no concilia o cuando una fila no alcanza la evidencia financiera exigida.
...
- La versión vigente del lector nativo es `ios-reader-recovery-2026.09.25.14` y la del motor es `rappi-shared-engine-2026.09.23.1`. Cualquier cambio de contrato exige actualizar ambas pruebas de bundle y la versión del lector.
...
Para la compuerta final de publicación, repetirlo con `MARCELITO_PDF_CORPUS_REQUIRE_CERTIFIED=1`. El informe debe contener los seis nombres esperados y la versión exacta del checkout actual (`ios-reader-recovery-2026.09.25.14` en esta revisión; compruébala siempre en `FinanceStore.readerVersion`). El método siempre debe venir del manifiesto privado; no transfieras el resultado `pdf-text` de PDF.js al lector iOS. En la auditoría nativa de estos seis Rappi, PDFKit no extrajo fechas ni importes firmados de la capa seleccionable, así que el método esperado en iOS es `vision-ocr`. Exige filas y controles exactos, cero filas rechazadas y conciliación válida; además, valida el flujo en un iPhone físico. La certificación del runner se basa en el conjunto exacto del manifiesto, no en un mínimo fijo de archivos: así el corpus Rappi de seis PDFs puede certificarse y el mismo gate sigue aceptando un corpus más amplio. Si falta una sola evidencia, detenerse en revisión y no subir a TestFlight.

Importante para esta entrega: la pantalla `Diagnóstico del lector` tiene un perfil `rappi-focused` de seis o más estados exclusivamente Rappi procesados por el lector local (`pdf-text` u `vision-ocr`), además del perfil general de 10 o más archivos. Puede exportar un JSON sanitizado para diagnosticar la lectura del dispositivo, pero ese informe no sustituye los goldens privados de filas y controles. Para la certificación fuerte, usar el runner con `MARCELITO_PDF_CORPUS_MANIFEST`, incluir `rowExpectations` exactos y convertir el log con `npm run pdf:native:report` a `docs/native-corpus-certification.json`. El workflow exige exactamente los 22 PDFs aprobados, la auditoría por fila y la prueba independiente OCR; no acepta `MARCELITO_NATIVE_CORPUS_CERTIFIED` ni un modo bootstrap como sustitutos.
