...
test("el parser rechaza encabezados administrativos con importes", () => {
  const text = [
    ...
    "02/08/2026 TOTAL IMPORTE CARGOS 12,345.67",
    "03/08/2026 estado de cue cuenta clabe 3,000.00",
    "04/08/2026 CARGO SUPERMERCADO 1,200.00",
  ].join("\n");
  const rows = extractTransactions(text, "BBVA", "sample-bank-bbva.pdf", "bank");
...
test("el parser no excluye un SPEI saliente a tercero como si fuera interno", () => {
  const [row] = extractTransactions("24/08/2026 SPEI ENVIADO A TERCERO 1,800.00 5,000.00", "BBVA", "sample-bank-bbva.pdf", "bank");
...
test("un traspaso textual a tercero queda como egreso hasta encontrar una contraparte propia", () => {
  const [row] = extractTransactions("24/08/2026 TRASPASO A TERCERO 1,800.00 5,000.00", "Santander", "sample-bank-period-3.pdf", "bank");
  assert.equal(row.kind, "purchase");
  assert.equal(row.flow, "expense");
});
...
test("un depósito bancario positivo se reconoce como ingreso real", () => {
  const statements = [bank("bbva", "BBVA", "agosto 2026")];
  const transactions = extractTransactions("24/08/2026 DEPÓSITO NÓMINA 8,000.00 10,000.00", "BBVA", "sample-bank-bbva.pdf", "bank");
  const result = runTransactionPipeline(transactions, statements);
...
test("el parser acepta fechas OCR y BBVA usa el primer importe, no el saldo corrido", () => {
  ...
  const repaired = extractTransactions([
    "05/AG0 TIENDA DE PRUEBA 125.00 1,230.94",
    "OBIAGO NOMINA EMPRESA 1,000.00 2,030.94",
  ].join("\n"), "BBVA", "sample-bank-bbva.pdf", "bank");
...
test("el estado BBVA completo reconstruye sus 11 filas y concilia los totales", () => {
  const text = [
    ...
    "10/AGO 10/AGO SPEI ENVIADO STP 500.00 1,230.94 1,230.94",
  ].join("\n");
  const rows = extractTransactions(text, "BBVA", "sample-bank-bbva.pdf", "bank");
...
test("la relectura con tipo corregido reconstruye filas y conserva evidencia", () => {
  const text = [
    ...
    "05/AGO COMPRA DE PRUEBA 125.00 1,230.94",
...
test("una fila bancaria sin saldo no usa referencias posteriores como importe", () => {
  const rows = extractTransactions([
    "30/JUL 30/JUL SPEI ENVIADO STP 500.00",
    "2206260binance Referencia 99887766 646",
    "111122223333444455",
    "MBAN010026073099887766 Sample User",
  ].join("\n"), "BBVA", "sample-bank-bbva.pdf", "bank");
...
test("OCR corrige 21600 a 216.00 cuando el saldo confirma el separador perdido", () => {
  ...
  ].join("\n"), "Santander", "sample-bank-period-1.pdf", "bank");
...
test("Santander selecciona el cargo o abono y no el saldo corrido", () => {
  ...
  const rows = extractTransactions(text, "Santander", "sample-bank-period-2.pdf", "bank");
...
test("Santander elige el saldo inicial que cuadra cuando el OCR duplica el gráfico", () => {
  const text = [
    ...
    "Otroscargos $6,416.11 Saldo inicial $562.79",
    "Detalle de movimientos",
    "16-JUL-2026 PAGO TRANSFERENCIA SPEI 6,416.11 5,559.79",
    "17-JUL-2026 NOMINA 3,618.74 2,765.42",
  ].join("\n");
  const summary = parseStatementSummary(text, "bank");
  assert.equal(summary.previousBalance, 5562.79);
  assert.equal(reconcileStatementImport("bank", summary, extractTransactions(text, "Santander", "sample-bank-period-3.pdf", "bank")).status, "valid");
});
...
test("Santander tolera que OCR lea RETIROS como RETROS sin perder el total", () => {
  ...
  assert.equal(summary.depositTotal, 4_922.25);
  assert.equal(summary.withdrawalTotal, 6_167.6);
  assert.equal(summary.cashBalance, 2_462.48);
});
...
test("Santander recupera separadores fusionados en los controles del resumen", () => {
  ...
  assert.equal(summary.previousBalance, 12_000);
  assert.equal(summary.depositTotal, 15_000);
  assert.equal(summary.withdrawalTotal, 13_000);
  assert.equal(summary.cashBalance, 14_000);
});
...
test("Santander corrige un 1 inicial alucinado cuando el saldo confirma el importe", () => {
  ...
  ].join("\n"), "Santander", "sample-bank-period-2.pdf", "bank");
  assert.equal(rows.length, 1);
...
test("Amex separa compras y pagos sin convertir el pago en gasto", () => {
  ...
  const rows = extractTransactions(text, "Amex", "sample-card-period-2.pdf", "card");
...
test("BBVA no se convierte en Santander por una contraparte dentro de movimientos", () => {
  ...
  assert.equal(detectSource(text, "sample-bank-bbva.pdf"), "BBVA");
...
test("la razón social Santander gana a una contraparte BBVA del resumen", () => {
  ...
  const evidence = detectSourceEvidence(text, "sample-bank-period-3.pdf");
...
test("marcadores legales conflictivos en el pie tampoco usan el nombre del archivo", () => {
  ...
  const detection = detectSourceEvidence(text, "sample-bank-period-3.pdf");
  assert.equal(detection.source, "Desconocido");
  assert.equal(detection.status, "unknown");
  assert.match(detection.evidence[0] ?? "", /conflictivos/);
});
...
test("el OCR con días imposibles no crea movimientos", () => {
  const rows = extractTransactions("75 de Agosto COMERCIO 48.00\n15 de Agosto COMERCIO 50.00", "Amex", "sample-card-period-3.pdf", "card");
  assert.deepEqual(rows.map((row) => row.amount), [-50]);
});

test("monto a diferir CR se registra como crédito y no como gasto", () => {
  const [row] = extractTransactions("27 de Junio MONTO A DIFERIR MESES EN AUTOMÁTICO 27,537.69 CR", "Amex", "sample-card-period-2.pdf", "card");
...
test("el resumen Amex conserva deuda comprometida, pago MSI y saldo pendiente", () => {
  const summary = parseStatementSummary([
    "0.00 - 27,537.69 + 37,213.42 = 967.57 1,350.00",
    "Pago Mínimo más meses sin intereses:$1,052.92",
    "Nuevas transacciones: 2,803.42",
    "Total Nuevos Cargos: 37,213.42",
    "Límite de Crédito 20,000.00 16,682.25",
    "Total de Plan de Meses sin Intereses 1,040.11 1,638.24",
  ].join("\n"), "card");
...
test("el resumen bancario descarta identificadores largos como saldo inicial", () => {
  ...
  assert.equal(summary.cashBalance, 1_230.94);
});
...
test("los totales bancarios declarados bloquean una importación que no concilia", () => {
  ...
  const rows = extractTransactions("01/08/2026 COMPRA 100.00 400.00", "BBVA", "sample-bank-bbva.pdf", "bank");
...
test("la zona de resumen Amex ignora el identificador de cuenta y conserva la ecuación", () => {
  ...
  assert.equal(summary.statementBalance, 3_996.62);
  assert.equal(summary.paymentForNoInterest, 3_996.62);
  assert.equal(summary.minimumPayment, 319.73);
...
test("la conciliación Amex usa subtotales nacional y extranjero como gasto real", () => {
  ...
  ].join("\n"), "Amex", "sample-card-period-3.pdf", "card");
  assert.equal(reconcileStatementImport("card", summary, rows).status, "valid");
});
...
test("la conciliación Amex descuenta créditos del subtotal doméstico y rechaza guías fechadas", () => {
  ...
  const rows = extractTransactions([
    "27 de Agosto americanexpress.com.mx Servicio al cliente 800 504 040 319.73",
...
test("un resumen histórico bloqueado no contamina deuda, gasto ni patrimonio", () => {
  const statements: Statement[] = [
    ...
    {
      ...card("amex-actual", "Amex", "28/07/2026 AL 27/08/2026", 3_317.75),
      reconciliationStatus: "valid",
      reconciliation: { status: "valid", tolerance: 0.05 },
      summary: {
        debtBalance: 3_317.75,
        statementBalance: 3_996.62,
        creditLimit: 20_000,
        creditAvailable: 16_682.25,
      },
    },
  ];
  ...
  assert.ok(Math.abs((metrics.debtTotal ?? 0) - 3_317.75) < 0.001);
  assert.equal(metrics.isProvisional, true);
  assert.equal(metrics.dataQuality.critical, true);
});
