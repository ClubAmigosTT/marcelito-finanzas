...
function normalizeBareBankSummaryAmount(raw: string, parsed: number) {
  ...
  // rescale the tightly bounded 7–8 digit shape; long account
...
function parseStatementSummary(text: string, kind: StatementKind): StatementSummary {
  ...
  if (kind !== "card") {
    ...
    // such as a bare leading token from a printed total).
  ...
  if (kind === "card") {
    ...
    // two synthetic money tokens”). These are future obligations, not new spend.
