...
function readLocalAccount(): LocalAccount | null {
  ...
  try {
    ...
    return saved?.username && saved.passwordHash ? saved : null;
  } catch {
    return null;
  }
}
...
function exportAuditDiagnostics(metrics: ReturnType<typeof buildFinanceMetrics>, statements: Statement[], auditRun: AuditRunRecord | null) {
  const payload = {
    ...
    statements: statements.map((statement) => ({
      ...
      mode: statement.mode,
      extractionProvider: statement.extractionProvider,
      extractionModel: statement.extractionModel,
      extractionPromptVersion: statement.extractionPromptVersion,
...
function DataQualityIndicator({ metrics }: { metrics: ReturnType<typeof buildFinanceMetrics> }) {
  ...
  const alert = quality.relevantReviewCount > 0 || quality.quarantinedStatementCount > 0 || quality.critical;
  const blockedReasons = [
    ["conciliación", quality.blockedReasonCounts.reconciliation],
    ["revisión", quality.blockedReasonCounts.review],
    ["OCR", quality.blockedReasonCounts.ocr],
    ["emisor", quality.blockedReasonCounts.source],
    ["evidencia", quality.blockedReasonCounts.evidence],
    ["tipo", quality.blockedReasonCounts.kind],
  ] as Array<readonly [string, number]>;
  const blockedReasonText = blockedReasons.filter(([, count]) => count > 0).map(([label, count]) => `${count} ${label}`).join(" · ");
