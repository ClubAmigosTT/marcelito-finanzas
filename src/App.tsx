...
function AuditDiagnostics({ metrics, auditRun }: { metrics: ReturnType<typeof buildFinanceMetrics>; auditRun: AuditRunRecord | null }) {
  ...
  return <details className="audit-diagnostics">
    ...
    {auditRun && <div className="audit-run-meta"><span>Auditoría {auditRun.id} · libro {auditRun.ledgerFingerprint}{auditRun.sourceFingerprints?.length ? ` · PDF${auditRun.sourceFingerprints.length === 1 ? "" : "s"} ${auditRun.sourceFingerprints.map((fingerprint) => fingerprint.slice(0, 8)).join(", ")}` : ""}</span><b className={`audit-status-${auditRun.status}`}>{auditRun.status === "passed" ? "Verificado" : auditRun.status === "warning" ? "Advertencias" : "Bloqueado"}</b></div>}
